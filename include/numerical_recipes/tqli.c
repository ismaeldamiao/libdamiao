...
The Art of Scientific Computing, 2nd Edition, 1992 p 480~481
      ...
      do{
         ...
         if(m != l){
            ...
            for(i = m-1; i >= l; i--){ // A plane rotation as in the
               f = s * e[i]; //  original QL, followed by Givens
               b = c * e[i]; // rotations to restore
               e[i+1] = r = pythag(f, g); // tridiagonal form.
